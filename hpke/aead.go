...
func (c *encdecCtx) Export(expCtx []byte, len uint16) []byte {
	return c.suite.labeledExpand(c.exporterSecret, []byte("sec"), expCtx, len)
}
...
func unmarshalContext(raw []byte) (*encdecCtx, error) {
	...
	Nh := int(c.suite.KdfID.Hash().Size())
...
func (c *sealCtx) Marshal() ([]byte, error) {
	...
	return append([]byte{0}, rawCtx...), nil
}
...
func (c *openCtx) Marshal() ([]byte, error) {
	...
	return append([]byte{1}, rawCtx...), nil
}
